...
async function cargarEvaluacionesMateria() {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      `)
      .eq("docente_id", estadoApp.usuarioId)
      .eq("seccion_id", estadoApp.seccionActual)
      .eq("lapso_id", lapsoId)
    ...
    estadoApp.evaluacionesMateria = data || [];
    ...
    if (data && data.length > 0) {
      mostrarTablaEstudiantes(data[0].id); // Mostrar estudiantes de la primera evaluación
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        lapso_id: estadoApp.lapsoActual,
        materia_id: estadoApp.materiaActual,
        tipo_evaluacion_id: tipoEval,
        instrumento_id: instrumento,
        nombre_evaluacion: nombre,
        fecha: new Date(fecha).toISOString(),
        valor_maximo: valorMaximo,
        porcentaje: porcentaje,
        descripcion: document.getElementById("input-descripcion-eval").value
      })
      .select();
