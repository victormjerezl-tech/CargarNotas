...
async function cargarEstudiantes() {
  ...
  const res = await fetch("/functions/v1/listar-estudiantes", {
    method: "POST",
    headers: {
  ...
  if (!data.ok) {
    console.error("Error listando estudiantes:", data.error);
    return;
  }

  window._estudiantes = data.estudiantes;
  renderizarTablaEstudiantes();
  renderizarPaginacionEstudiantes(data.total);
}
